...
// @include      http://dynamic.cloud.vip.xunlei.com/cloud*
...
// @updateURL https://github.com/miniers/ThunderLixianExporter/raw/miniers-mutilServer/ThunderLixianExporter.user.js
// ==/UserScript==
...
function tle_wrapper() {
    ...
    TLE.exporter = {
        ...
        "YAAW": function(todown, path) {
          
            if (TLE.getConfig("TLE_aria2_jsonrpcs")) {
    ...
    (function(TLE) {
        ...
        TLE.down = function(_this, _do, rpc_path) {
        ...
        TLE.batch_down = function(_this, _do, rpc_path) {
        ...
        TLE.bt_down = function(_this, _do, rpc_path) {
        ...
        TLE.bt_down_one = function(_this, _do, rpc_path) {
        ...
        TLE.addrpc = function() {
            var setting = $('.rpc_setting');
            setting.append('<li class="json_rpc_list"><input type="text" class="jsonrpc_name" style="width: 60px" value=""/>: <input type="text" class="jsonrpc_path" style="width: 350px" value=""/> <a onclick="TLE.removerpc(event);">删除</a></li>')
        }

        TLE.removerpc = function(e) {
            var aria2_jsonrpcs = JSON.parse(TLE.getConfig("TLE_aria2_jsonrpcs") || '{}');
            var _this = $(e.target).parent();
            var name = _this.find('.jsonrpc_name').val();
            _this.remove();
            if (aria2_jsonrpcs[name]) {
                delete aria2_jsonrpcs[name];
            }
            TLE.setConfig("TLE_aria2_jsonrpcs", JSON.stringify(aria2_jsonrpcs));
        }
        ...
        function init() {
            ...
            $("#setting_main_tpl").text($("#setting_main_tpl").text().replace(/(<\/div>\s+<div class="btnin">)/,
                ...
                '<li onclick="TLE.addrpc();"><b>Aria2 JSON-RPC Path</b></li>' +
                (function() {
                    var aria2_jsonrpcs = JSON.parse(TLE.getConfig("TLE_aria2_jsonrpcs") || '{"mac":"http://127.0.0.1:6800/jsonrpc"}');
                    var str = '',
                        data;
                    for (var key in aria2_jsonrpcs) {
                        data = aria2_jsonrpcs[key]
                        str += '<li class="json_rpc_list"><input type="text" class="jsonrpc_name" style="width: 60px" value="' + key + '"/>: <input type="text" class="jsonrpc_path" style="width: 350px" value="' + aria2_jsonrpcs[key] + '"/> <a onclick="TLE.removerpc(event);">删除</a></li></li>';
                    }
                    return str;
                })() +
                '</ul>' +
            ...
            set_notice_submit = function(f) {
                ...
                var rpc_lists = $(".json_rpc_list"),
                    rpc_setting = {},tmpDom;
                for (var i = 0; i < rpc_lists.length; i++) {
                    tmpDom = rpc_lists.eq(i);
                    rpc_setting[tmpDom.find('.jsonrpc_name').val()] = tmpDom.find('.jsonrpc_path').val()
                }

                var jsonrpc_path = JSON.stringify(rpc_setting);
                if (TLE.getConfig("TLE_exporter") != config_str || TLE.getConfig("TLE_aria2_jsonrpcs") != jsonrpc_path) {
                    TLE.setConfig("TLE_exporter", config_str);
                    TLE.setConfig("TLE_aria2_jsonrpcs", jsonrpc_path);
                    TS2.show('设置已生效', 1);
                    setTimeout(function() {
                        setting.hide();
            ...
            function exporter_anchors(type) {
                ...
                var enabled_exporter = TLE.getConfig("TLE_exporter").split("|");
                var str = '';
                var rpc_setting = JSON.parse(TLE.getConfig("TLE_aria2_jsonrpcs") || '{}');
                $.each(TLE.exporter, function(n, f) {
                    if (enabled_exporter.indexOf(n) == -1) return;
                    if (n == 'YAAW') {
                        for (var key in rpc_setting) {
                            str += ('<a href="#" title="' + key + '" onmouseover="this.className=\'sel_on\'" onmouseout="this.className=\'\'" onclick="' + type + '(this, TLE.exporter[\'' + n + '\'],\'' + rpc_setting[key] + '\')">' + key + '</a>');
                        }
                    } else {
